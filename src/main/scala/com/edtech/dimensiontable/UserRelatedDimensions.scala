...
object UserRelatedDimensions extends App {
  ...
  if (dimDeviceCount > 0) {
     ...
     */
    val dimDeviceTypeDf1 = spark.sql("""select 
   ...
   */
   ...
   if (dimUserDetailsCount > 0) {
    ...
    val transformedDf = spark.sql("""Select concat(user_id,'-',user_name)  as  user_details_key, userid, user_name, user_email
